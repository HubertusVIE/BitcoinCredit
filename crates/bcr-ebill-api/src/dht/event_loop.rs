...
use libp2p::{PeerId, gossipsub, relay};
                            ...
                            match event.id {
                                    ...
                                    if let Ok(block) = from_slice::<BillBlock>(&event.message) {
                                            ...
                                            self.bill_blockchain_store.get_chain(bill_id).await
                                        {
                                            ...
                                            if chain.is_chain_valid() {
                                                ...
                                                {
                                                    error!(
                                                        "Could not add block (id: {}, hash: {}) for bill {bill_id}: {e}",
                                                        block.id, block.hash
                                                    );
                                                }
                                            }
                                ...
                                GossipsubEventId::BillBlockchain => {
                                        ...
                                        from_slice::<BillBlockchain>(&event.message)
                                    {
                                        ...
                                        {
                                            info!(
                                                "Received blockchain for bill: {bill_id} from peer: {node_id}, local block height: {}, remote block height: {}",
                                                local_chain.block_height(),
                                                remote_chain.block_height()
                                            );
    ...
    async fn handle_command(&mut self, command: Command) -> Result<()> {
        match command {
            ...
            Command::PutRecord { key, value } => {
                ...
                let relay_peer_id: PeerId = get_config()
