use super::behaviour::{
    ...
    Event, FileResponse, ParsedInboundFileRequest, file_request_for_bill,
    file_request_for_bill_attachment, file_request_for_bill_keys, file_request_for_company_chain,
    file_request_for_company_data, file_request_for_company_keys, file_request_for_company_logo,
    file_request_for_company_proof, parse_inbound_file_request,
};
...
use crate::constants::{BILL_PREFIX, BILLS_PREFIX, COMPANIES_PREFIX, COMPANY_PREFIX};
use crate::data::{
    bill::BillKeys,
    company::{Company, CompanyKeys},
};
use crate::persistence::bill::{
    BillChainStoreApi, BillStoreApi, bill_chain_from_bytes, bill_chain_to_bytes,
    bill_keys_from_bytes, bill_keys_to_bytes,
};
use crate::persistence::company::{
    CompanyChainStoreApi, CompanyStoreApi, company_chain_from_bytes, company_chain_to_bytes,
...
use future::{BoxFuture, try_join_all};
        ...
        match futures::future::select_ok(requests).await {
            ...
            Ok(file_content) => {
                ...
                if hash != remote_hash.as_str() {
                    return Err(super::Error::FileHashesDidNotMatch(format!(
                        "Get Company File: Hashes didn't match for company {company_id} and file name {file_name}, remote: {remote_hash}, local: {hash}"
                    )));
                }
                let encrypted_bytes =
        ...
        if let Ok(companies_record) = self.get_record(node_request.clone()).await {
            ...
            if !companies_for_node.is_empty() {
                match from_slice::<Vec<String>>(&companies_for_node) {
                    ...
                    Err(e) => {
                        error!(
                            "Could not parse company data in dht for {node_id} when trying to remove company {company_id}: {e}"
                        );
                    }
                }
            }
    ...
    async fn request_bill_attachment_data(
        ...
        match futures::future::select_ok(requests).await {
            ...
            Ok(file_content) => {
                ...
                if hash != remote_hash.as_str() {
                    return Err(super::Error::FileHashesDidNotMatch(format!(
                        "Get Bill Attachment: Hashes didn't match for bill {bill_id} and file name {file_name}, remote: {remote_hash}, local: {hash}"
                    )));
                }
                ...
                match event {
                    CompanyEvent::AddSignatory => {
                        ...
                        if let Ok(identity) = self.identity_store.get().await {
                            ...
                            if signatory == identity.node_id {
                                info!(
                                    "Got from DHT, that we were added to company {company_id} - refreshing data from the network"
                                );
                                if let Err(e) = self
                    ...
                    CompanyEvent::RemoveSignatory => {
                        ...
                        if let Ok(identity) = self.identity_store.get().await {
                            ...
                            if signatory == identity.node_id {
                                info!(
                                    "Got from DHT, that we were removed from company {company_id} - deleting company locally"
                                );
...
mod tests {
    ...
    fn get_client_chan_stores(
        mock_bill_storage: MockBillStoreApiMock,
        mock_bill_blockchain_storage: MockBillChainStoreApiMock,
        mock_company_storage: MockCompanyStoreApiMock,
        mock_company_blockchain_storage: MockCompanyChainStoreApiMock,
    ...
    fn get_storages() -> (
        MockBillStoreApiMock,
        MockBillChainStoreApiMock,
        ...
        (
            MockBillStoreApiMock::new(),
            MockBillChainStoreApiMock::new(),
